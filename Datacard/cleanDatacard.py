  ...
  with open(opts.datacard) as inFile:
    for line in inFile.readlines():
      ...
      if vals[0] == 'bin' and len(cats)==0:
        ...
        continue
      if vals[1]!='lnN': 
        if not line.count('hggpdfsmrel_13TeV_2017_'): line = line.replace('hggpdfsmrel_13TeV_','hggpdfsmrel_13TeV_2017_')
        if not line.count('13TeV_2017_bkgshape'): line = line.replace('13TeV_bkgshape','13TeV_2017_bkgshape')
        if line.count('pdfindex') and not line.count('_13TeV_2017'): line = line.replace('_13TeV','_13TeV_2017')
        outFile.write('%s'%line)
        continue
      print
