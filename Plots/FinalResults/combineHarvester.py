...
specOpts.add_option("--stxsFreezeNuisances",default="QCDscale_qqH,pdf_Higgs_qqH,pdf_Higgs_ggH",help="The nuisances to be frozen when doing the STXS measurements")
...
def writePostamble(sub_file, exec_line):
  ...
  if not opts.dryRun and opts.queue:
    ...
    if (opts.batch == "IC") : system('qsub -q %s -l h_vmem=10G -o %s.log -e %s.err %s > out.txt'%(opts.queue,os.path.abspath(sub_file.name),os.path.abspath(sub_file.name),os.path.abspath(sub_file.name))) #FIXME
